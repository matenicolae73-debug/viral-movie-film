...
export default function Home() {
  ...
  function downloadVideo(sceneId: number) {
    const url = videoUrls[sceneId]; if (!url) { setStatus("Video is not ready yet."); return; }
  ...
  return <main className="app-shell">
    ...
    <div className="layout">
      ...
      <section className="content">
        <div className="hero-image"><img src="/hero-dashboard.png" alt="ViralMovie AI cinematic studio"/><div className="hero-copy"><h1>AI Makes <span>Films</span> Online</h1><p>Create stunning movies, viral videos and epic stories with the power of AI. Turn your ideas into reality!</p><div className="hero-badges"><span>◉ Text to Video</span><span>◉ Multiple Scenes</span><span>◉ HD Quality</span><span>✣ Social Ready</span></div></div></div>
        <div className="workspace">
          <div className="main-column">
            ...
            <section className="card" id="stage-2"><div className="section-head"><h2>👤 Characters</h2><span>CONSISTENCY</span></div><button className="secondary" onClick={openCharacters}>Create Characters</button>{characters.length > 0 && <div className="character-row">{characters.map(c => <div className="character" key={c}><div className="avatar">◉</div><b>{c}</b><small>Reference ready</small></div>)}</div>}</section>

            <section className="card" id="stage-3"><div className="section-head"><h2>▣ Storyboard</h2><span>{story?.sceneCount || 0} SCENES</span></div>{!story ? <div className="info-box">Generate the Movie first.</div> : <div className="scene-grid">{story.scenes.map(scene => <div className="scene-card" key={scene.id}><div className="scene-thumb">🎞️</div><div><b>Scene {scene.id}</b><p>{scene.prompt}</p><button onClick={() => { setSelectedScene(scene); go(4); }}>{generated[scene.id] ? "✓ Submitted" : "Open Scene"}</button></div></div>)}</div>}</section>

            <section className="card" id="stage-4"><div className="section-head"><h2>🎥 AI Video</h2><span>{selectedScene && videoState[selectedScene.id] ? videoState[selectedScene.id] : "READY"}</span></div>{selectedScene ? <><div className="info-box"><b>Scene {selectedScene.id}</b><br/><span>{selectedScene.prompt}</span></div><button className="generate" onClick={() => generateScene(selectedScene)}>{generated[selectedScene.id] ? "↻ Generate Again" : "✦ Generate Scene"}</button>{readyUrl && <div className="video-box"><video controls playsInline src={readyUrl}/><div className="video-actions"><button className="download" onClick={() => downloadVideo(selectedScene.id)}>⇩ Download Video</button><button className="preview" onClick={() => document.getElementById("preview")?.scrollIntoView({ behavior: "smooth" })}>◉ Preview</button></div><div className="share-title">Share on Social Media</div><div className="socials"><button onClick={() => shareVideo("facebook", selectedScene.id)}>f <span>Facebook</span></button><button onClick={() => shareVideo("instagram", selectedScene.id)}>◎ <span>Instagram</span></button><button onClick={() => shareVideo("tiktok", selectedScene.id)}>♪ <span>TikTok</span></button><button onClick={() => shareVideo("youtube", selectedScene.id)}>▶ <span>YouTube</span></button><button onClick={() => shareVideo("share", selectedScene.id)}>↗ <span>Share</span></button></div></div>}</> : <div className="info-box">Choose a scene from Storyboard first.</div>}</section>
          ...
          <aside className="right-column" id="preview"><section className="card preview-card"><div className="section-head"><h2>▶ Movie Preview</h2><span>LIVE</span></div>{readyUrl ? <video controls playsInline src={readyUrl}/> : <div className="preview-art"><img src="/hero-dashboard.png" alt="Movie preview"/><div className="play">▶</div></div>}<div className="preview-actions"><button className="download" disabled={!readyUrl} onClick={() => selectedScene && downloadVideo(selectedScene.id)}>⇩ Download Video</button><button className="preview" disabled={!readyUrl} onClick={() => readyUrl && window.open(readyUrl, "_blank")}>◉ Preview</button></div>{readyUrl && selectedScene && <><div className="share-title">Share on Social Media</div><div className="socials"><button onClick={() => shareVideo("facebook", selectedScene.id)}>f <span>Facebook</span></button><button onClick={() => shareVideo("instagram", selectedScene.id)}>◎ <span>Instagram</span></button><button onClick={() => shareVideo("tiktok", selectedScene.id)}>♪ <span>TikTok</span></button><button onClick={() => shareVideo("youtube", selectedScene.id)}>▶ <span>YouTube</span></button></div></>}</section>
