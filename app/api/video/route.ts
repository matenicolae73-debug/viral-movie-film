...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`https://queue.fal.run/${MODEL}`, { method: "POST", headers: { Authorization: `Key ${key}`, "Content-Type": "application/json", Accept: "application/json", "X-Fal-Store-IO": "1" }, body: JSON.stringify({ prompt, aspect_ratio, duration: 5, resolution: "540p", audio: true }), cache: "no-store" });
