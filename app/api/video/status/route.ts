...
export async function POST(request: Request) {
  try {
    ...
    if (!r.ok) return NextResponse.json({ ok: false, error: data?.detail || data?.message || data?.error || "The service returned an error. Please try again.", falStatus: r.status, data }, { status: r.status });
