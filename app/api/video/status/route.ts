...
export async function POST(request: Request) {
  try {
    ...
    if (!key) return NextResponse.json({ ok: false, error: "FAL_KEY is missing." }, { status: 500 });
    const custom = action === "result" ? body?.responseUrl : body?.statusUrl;
    const url = custom || `https://queue.fal.run/${MODEL}/requests/${encodeURIComponent(requestId)}${action === "result" ? "" : "/status?logs=1"}`;
    ...
    if (!r.ok) return NextResponse.json({ ok: false, error: data?.detail || data?.message || data?.error || `fal.ai HTTP ${r.status}`, falStatus: r.status, data }, { status: r.status });
    const normalized = { ...data, status: data?.status || data?.state || data?.data?.status || data?.data?.state || null, error: data?.error || data?.detail || data?.data?.error || null, logs: Array.isArray(data?.logs) ? data.logs : [] };
